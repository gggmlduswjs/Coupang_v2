...
def render(selected_account, accounts_df, account_names):
    ...
    # ── 2 탭 ──
    _ord_tab1, _ord_tab4 = st.tabs(["결제완료", "출고/극동"])

    # ══════════════════════════════════════
    # 탭1: 결제완료 (ACCEPT) + 발주서 + 상품준비중 처리
    # ══════════════════════════════════════
    with _ord_tab1:
        st.caption("WING API 실시간 · 체크박스로 선택 → 발주서 생성 / 상품준비중 처리")
        ...
        if _t1_data.empty:
            ...
            if True:
                ...
                # ── 발주서 생성 ──
                with st.expander(f"📋 발주서 ({len(_dist_orders)}건)", expanded=True):
                    if _dist_orders.empty:
                        ...
                        _isbn_missing = len(_dist_df) - _isbn_found
                        if _isbn_missing > 0:
                            st.caption(f"ℹ️ ISBN 없음: {_isbn_missing}/{len(_dist_df)}건 (삭제된 상품 또는 세트물은 정상)")
                        ...
                        AgGrid(_show_agg, gridOptions=grid_opts2, height=500, theme="streamlit", key="dist_grid")

                st.divider()

                # ── 상품준비중 처리 (선택된 주문만) ──
                st.subheader("상품준비중 처리")
                st.info(f"선택한 {_sel_boxes}건(묶음배송)을 상품준비중(INSTRUCT)으로 변경합니다. (쿠팡 Wing에 실제 반영)")

                if st.button(f"선택한 주문 상품준비중 처리 ({_sel_boxes}건)", type="primary", key="btn_ack_selected"):
                    _acct_groups = _sel_data.groupby("_account_id")
                    _total_success = 0
                    _total_fail = 0

...
반품 관리 페이지
================
WING API 실시간 반품/취소 목록 조회 + 입고확인/반품승인/회수송장등록 처리.
"""
...
def render(selected_account, accounts_df, account_names):
    ...
    with c1:
        if st.button("반품 새로고침", key="btn_ret_refresh", use_container_width=True,
                     type="primary", help="WING API에서 실시간 반품 조회"):
            _clear_return_cache()
            st.rerun()
    with c2:
    ...
    if _all.empty:
        st.info("반품/취소 건이 없습니다. '반품 새로고침'을 눌러 조회하세요.")
        return
    ...
    # ── KPI 카드 ──
    _total = len(df)
    _pending = len(df[df["_receipt_status"].isin(["RELEASE_STOP_UNCHECKED", "RETURNS_UNCHECKED"])]) if "_receipt_status" in df.columns else 0
    _warehouse = len(df[df["_receipt_status"] == "VENDOR_WAREHOUSE_CONFIRM"]) if "_receipt_status" in df.columns else 0
    _completed = len(df[df["_receipt_status"] == "RETURNS_COMPLETED"]) if "_receipt_status" in df.columns else 0

    # 귀책 분포 (필터 전 전체 기준)
    _cust = len(_all[_all["귀책"] == "고객"])
    _vendor = len(_all[_all["귀책"] == "셀러"])
    _coup = len(_all[_all["귀책"] == "쿠팡"])
